...
public class Plateau {
	...
	public ArrayList<Case> getListeCase() {
		return plateau; // retourne l'entiÃ¨reté de la liste de case
	}

	// Lie la positionJoueur avec la case correspondante
	public Case getTypeCase(int index) { // prendre une case avec en paramÃ¨tre son index -> fait en sorte que la méthode accepte des chiffres extérieurs
		return plateau.get(index); // retourne une case => index
	}
	/*cette méthode est la mÃªme que dessus, cela me renvoie l'index de la case avec ce qu'elle correspond public 
	 * Case idCasePlateau(int idCase) { //méthode avec la classe Case nommée prenant en paramÃ¨tre int idCase donc l'index return
	...
	public void initPlateau() { // initation du tableau avec des cases vides et une case Start
		...
		for (int i = 0; i < 20; i++) { // tableau de 64 cases + instance du plateau de jeu en dur.
			plateau.add(new EmptyCase()); // ajoute case vide (de index 0 Ã  fin du plateau tant que celle-ci n'est pas modifiée)
		}

		plateau.set(0, new StartCase()); // renvoie la classe StartCase() qui est 'Start Case' = index 0
		// La méthode set(int index, Element E) met Ã  jour l'élément de l'index spécifié avec l'élément E donné -> public E set(int index, Element E)
