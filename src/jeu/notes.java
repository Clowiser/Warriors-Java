...
_______________________
 ...
 tant que (!isRunning) { // ici !boolean donc différent de false soit true
  	...
  	[...]
  	case 3: 
  	isRunning = true; -> en sélectionnant 3, isRunning devient true et donc arrÃªte la boucle ? 
 } 
 ...
 -> Un constructeur est une méthode d'une classe donnée, servant à créer des objets.
	...
	- il a le mÃªme nom que la classe
...
-> cast : forcer le compilateur à considérer une variable comme étant dâ€™un type qui nâ€™est pas le type déclaré ou le type réel de la variable.
...
Interface : 
	 * c'est une collection de méthodes QUE abstraites qui seront redéfinies par une classe // que l'on veut appliquer à une classe, sans pour autant l'intégrer à une classe, au modèle de ces objets
	 * ce n'est PAS une Class => on ne peut pas créer d'objet à partir d'une interface
	 * on ne peut pas créer le constructeur (logique)
	 * il n'y a plus de notion d'héritage -> on fait de l'implémentation via implements (on adapte ce code à cette class)
	 * 
	 * l'héritage multiple n'existe pas -> donc s'il on veut plusieurs fonctions partagées, il faut des interfaces = un seul héritage/pls interface
	 * Par le fait que case soit en abstract, "elle est déjà une interface"
	 * 
	 * Alors comment choisir entre les méthodes abstraites et l'interface ?
	 * Choix en faveur de la cohérence de vos modèles d'objets = classes abstraites ou interface, globalement, ça s'utilise de la même manière mais ce n'est pas la même chose.
	 * Dans une classe - utilise pour imposr un comportement global : on va pouvoir définir des attributs, constructeurs (instancier des nouveaux objets) et méthodes (avec corps ou sans corps, soit abstraite)
	 * Dans une interface : elle prend plus le rôle d'un ensemble d'outils
	 * 
	 * En bref, une interface, c'est un ensemble de méthodes que l'on veut injecter et forcer à redéfinir très spécifiquement dans une classe.
...
- ajouter "Passer au joueur suivant" ou "case événement" quand le J1 a fini son tour (1 lancer de dés)
 ...
 REGLES :
	...
	// ennemi : un ennemi apparaÃ®t => enclenche l'action combat()
